...
def constrained_generation_dfs(
    dynamic_vocab_cache,
    inputs_ids,
    next_token_logits,
    ...
    for i in range(next_token_logits.size()[0]):
        ...
        current_to_possible_tokens = compute_dynamic_vocab_dfs(
            dynamic_vocab_cache, preds, current_to_possible_tokens_map, tokenizer
        )
        logits_softmax_applied = constrained_logits_log_softmax_for_generation(
            current_to_possible_tokens, i, next_token_logits
        )
        logits_rows.append(logits_softmax_applied)
    return torch.stack(logits_rows)


def constrained_generation_bfs(
    dynamic_vocab_cache,
    inputs_ids,
    next_token_logits,
    current_to_possible_tokens_map,
    ...
    for i in range(next_token_logits.size()[0]):
        preds = inputs_ids[i].tolist()
        current_to_possible_tokens = compute_dynamic_vocab_bfs(
            dynamic_vocab_cache,
            preds,
            current_to_possible_tokens_map,
            set_all_labels,
...
def compute_dynamic_vocab_dfs(
    dynamic_vocab_cache, preds, current_to_possible_tokens_map_parent_child, tokenizer
):
    ...
    if preds == [18134]:
        return [
            value[0] for value in current_to_possible_tokens_map_parent_child["Root"]
        ]
    ...
    for pred in preds_text:
        if pred != "Pop":
            stack.append(
                pred
            )  # Continue until decoder cannot find the label in parent_child_map
        elif stack:
            stack.pop()
    ...
    if stack:
        ...
        return [5777]
    if y_current in current_to_possible_tokens_map_parent_child and (
        not stack or stack[-1] != y_current
    ):  # the and is for RCV1V1 we have C17 but C171 also !!!
        if preds[-1] != 3 and preds[-1] != 2:
            possible_tokens = [
                e[0] for e in current_to_possible_tokens_map_parent_child[y_current]
            ]
            ...
            result = [
                lst
                for lst in list_of_childs
                if lst[: len(current_tokens)] == current_tokens
            ]
            next_ones = [
                e[len(current_tokens)] for e in result if len(e) > len(current_tokens)
            ]
            if next_ones:
...
def constrained_logits_log_softmax(current_to_possible_tokens, i, logits, prob_f):
    ...
    """
    logits_softmax_applied = torch.full(
        logits[i].size(), math.log(1e-32), device=logits.device
    )
    logits_softmax_applied[current_to_possible_tokens] = prob_f(
        logits[i, current_to_possible_tokens]
    )
    return logits_softmax_applied


def constrained_logits_log_softmax_for_generation(
    current_to_possible_tokens, i, logits
):
    """
    ...
    logits_softmax_applied[current_to_possible_tokens] = logits[
        i, current_to_possible_tokens
    ]
    return logits_softmax_applied
...
def encode_text_data(parent_child, tokenizer, is_dfs=True):
    ...
    for key, value_list in parent_child.items():
        encoded_values = []
        [
            encoded_values.append(tokenizer(value, add_special_tokens=False).input_ids)
        ...
        if key not in encoded_map:
            ...
            if is_dfs and key != "Root":
                encoded_values.append(
                    [5777]
                )  # Pop is always possible but Root. 5777 is token number for "Pop"
...
def constrained_loss_dfs(
    model, loss_f, logits_batch, target_batch, current_to_possible_tokens_map, tokenizer
):
    ...
    for j in range(batch_size):
        ...
        for i in range(len_seq):
            if target[i].unsqueeze(0) == 0:
                break
            logits_softmax_applied = constrained_logits_log_softmax(
                current_to_possible_tokens, i, logits, prob_f
            )
            current_pred_token = torch.argmax(logits_softmax_applied).item()
            preds.append(current_pred_token)
            current_to_possible_tokens = compute_dynamic_vocab_dfs(
                model.dynamic_vocab_cache,
                preds,
                current_to_possible_tokens_map,
                tokenizer,
            )
...
def constrained_loss_bfs(
    ...
    for j in range(batch_size):
        ...
        preds = []
        len_seq = target.size()[0]
        current_to_possible_tokens = [
            l[0] for l in current_to_possible_tokens_map["Root"]
        ]  # First level label tokens
        for i in range(len_seq):
            if target[i].unsqueeze(0) == 0:
                break
            logits_softmax_applied = constrained_logits_log_softmax(
...
def compute_dynamic_vocab_bfs(
    ...
    if preds and preds[-1] == 0:
        ...
        return next_ones
    preds_text = (
        tokenizer.decode(preds, skip_special_tokens=True)
        .replace("/", " /")
        .replace("-", " -")
    )
    ...
    if (
        (
            y_current in set_all_labels
            and (len(preds_text_list) <= 1 or y_current not in preds_text_list[0:-1])
        )
        or preds[-1] == 3
        or preds[-1] == 2
    ):
        ...
        [
            ...
            for value in preds_text_by_level[current_level - 1].split(" -")
        ]
